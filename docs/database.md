...
| invite_code | text | NOT NULL、DEFAULT自動生成 | 参加URLに使用する8文字のランダム文字列 |
...
- `invite_code` にUNIQUE制約は不要。参加URLには `room_id` と `invite_code` の両方が含まれ、`join_room()` も両方を検証するため、異なるルームが同じ `invite_code` を持っていても安全性・機能に影響しない
...
> **REPLICA IDENTITY FULL について：** 現時点ではどちらのテーブルもINSERT専用のため、技術的にはFULLでなくても動作する。ただし将来的にUPDATE/DELETEを追加した場合にFULLが必要になるため、あらかじめ設定している。
...
`volatile`（デフォルト）を明示している。`stable` にするとPostgreSQLが同一ステートメント内で結果をキャッシュするため、`INSERT INTO rooms ... RETURNING` の中で `handle_new_room` トリガーが `room_members` に行を追加しても、RETURNING句のRLS評価時に古いキャッシュ（メンバーでない）が使われて行が返らなくなる。
...
| 操作 | ロール | 条件 | 説明 |
|---|---|---|---|
| SELECT | authenticated | `owner_id = auth.uid()` | オーナーは自分のルームを常に読める（作成直後も含む） |
| SELECT | authenticated | `is_room_member(id)` | メンバーも読める。`invite_code` の漏洩を防ぐため認証必須 |
| INSERT | authenticated | `owner_id = (select auth.uid())` | 自分をオーナーにしたルームのみ作成可 |
| UPDATE | authenticated | `(select auth.uid()) = owner_id` | オーナーのみ変更可。変更不可カラムはトリガーで保護 |
| DELETE | — | 不可 | ルームは削除しない |

> `rooms: owner select` と `rooms: members select` の2つのSELECTポリシーがOR条件で機能する。
> 参加前のルーム情報は `get_room_preview()` RPC を使うこと。`supabase.from('rooms')` は参加済みルームまたはオーナーのルームのみ取得できる。
...
as $$
begin
  if nullif(trim(new.raw_user_meta_data->>'display_name'), '') is null then
...
-- rooms
-- オーナーは作成直後（handle_new_roomトリガー実行前）でも自分のルームを参照できる必要があるため
-- owner select と members select の2ポリシーをOR条件で使用する
create policy "rooms: owner select"
...
-- (select auth.uid()) を使用する理由：
-- auth.uid() を直接使うとPostgreSQLがSTABLE関数としてキャッシュし、
-- INSERT+RETURNINGのコンテキストで誤評価される場合があるため
...
- **表示名なしでのサインアップ不可**：`handle_new_user` トリガーが `nullif(trim(...), '') is null` で空文字・空白のみも拒否するため、フロントエンドで必須入力として制御すること
